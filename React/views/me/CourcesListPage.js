...
class CourcesListPage extends Component {
    ...
    _getCollectList = function (page) {
        var context = this;
        ReactModule.fetch("get",{api:"/course/list.do?start="+page+"&pageSize=10"},function (error, response) {
            if (error){
                currentPage -=1;
            }else {
                console.log("ihg cources:", response);
                console.log("ihg currentPage:", currentPage);
                ...
                console.log("ihg videoList:", videoList);
                context.setState({
                    videos: videoList
                });
            }
